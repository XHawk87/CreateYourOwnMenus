 ...
 * @author XHawk87
 */
public class MenuScriptInsertCommand implements IMenuCommand {

    @Override
    public String getUsage() {
        return "/menu script insert [index] [text] - Inserts the given text at line with the given index (0 for before first) in the held item's lore";
    }
        ...
        if (sender instanceof Player) {
            ...
            try {
                ...
                if (index > loreStrings.size()) {
                    sender.sendMessage("The index must be at most the number of lines in the lore (" + loreStrings.size() + "): " + indexString);
                    return true;
                }
